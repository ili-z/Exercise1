...
<html>
  ...
  <body>
    <form action="javascript:;" method="post">
		...
		二级分类:<select name="seleTwo" id="seleTwo" onchange="change3();">
			<c:forEach items="${product1}" var="product1">
				<option value="${product1.id}" selected="selected">${product1.name}</option>
			</c:forEach>
			<option  value="0" selected="selected">--请选择--</option>
		</select>	
		三级分类:<select name="seleThree" id="seleThree">
			<c:forEach items="${product1}" var="product1">
				<option class="san" value="${product1.id}" selected="selected">${product1.name}</option>
			</c:forEach>
